use idwc::{
    TranspileErrorKind, TranspileMode, TranspileOptions, transpile, transpile_with_options,
};

fn stupid(source: &str) -> String {
    transpile_with_options(source, TranspileOptions::new(TranspileMode::Stupid)).unwrap()
}
...
/// Explicit strict options must retain the stable default translation contract.
#[test]
fn explicit_strict_mode_matches_transpile() {
    let source = include_str!("../examples/variables.rs");
    assert_eq!(
        transpile(source).unwrap(),
        transpile_with_options(source, TranspileOptions::default()).unwrap()
    );
}
...
fn stupid_mode_generates_readable_c() {
    let generated = stupid(
        r#"fn main() {
            let score = 1;
            { let score = 2; println!("分數：{}", score); }
            println!("{}", score);
            println!("{}", r#int(3));
        }
        fn r#int(printf: i32) -> i32 { printf }"#,
    );
    assert!(generated.contains("Generated by IdwC --stupid"));
    assert!(generated.contains("printf(u8\"分數：%d\\n\", score_2);"));
    assert!(generated.contains("const int score = 1;"));
    assert!(generated.contains("const int score_2 = 2;"));
    assert!(generated.contains("int int_value(int printf_value)"));
    assert!(!generated.contains("idwc_v"));
    assert!(!generated.contains("idwc_t"));
...
fn stupid_mode_omits_runtime_helpers_and_checks() {
    let generated = stupid(include_str!("../examples/stupid_stdin.rs"));
    for rejected in [
        "static ",
        "struct ",
        "idwc_fail",
        ...
        assert!(!generated.contains(rejected), "unexpected `{rejected}`");
    }
    assert!(generated.contains("scanf(\"%lf\", &kg);"));
    assert!(generated.contains("puts(u8\"過瘦\");"));
}

/// The checked-in Stupid Mode example must remain identical to generated output.
#[test]
fn stupid_mode_example_matches_generated_output() {
    assert_eq!(
        stupid(include_str!("../examples/stupid_stdin.rs")),
        include_str!("../examples/stupid_stdin.c")
    );
}
